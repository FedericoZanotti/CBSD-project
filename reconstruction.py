...
Original file is located at
    https://colab.research.google.com/drive/155vTR2hyYPZzVbeDoiJsfn0UgBfOXL10
"""
...
def create_structure(X_train, X_test, y_train, y_test, LIGHT):
  ...
  for m in model_list:
    ...
    for k in range(y_train.shape[1]):
      ...
      for i in range(1,X_train.shape[1]):
        ...
        # print("computing for combination: ", comb)
        for j in range(len(comb)):
          model = m
          cl=list(comb[j])
          model.fit(X_train[:,cl], y_train[:,k])
          y_pred = model.predict(X_test[:, cl])
          result[str(comb[j])] = my_accuracy(y_pred, y_test[:,k])
      all_dict[k] = result
...
def get_max(d, flag_max=True):
  ...
  for k1,v1 in d.items():
    ...
    for k, v in d[k1].items():
      ...
      # print(mx[0])
      # print(maximum[k])
        # print(f"max with {maximum} and model {best_model}")
